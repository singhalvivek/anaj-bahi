...
### Phase 4 — Cloud sync & backup (FastAPI + Postgres)

- **Goal:** When signal returns, local bills/farmers/payments sync to a personal cloud backend (backup + restore on a new device); creating/editing while offline queues changes and flushes automatically on reconnect.
...
- **Independent slices (parallel build units):**
  - `slice-a` (backend) — FastAPI sync service + Postgres schema + `uv` project under `backend/`; push/pull endpoints with last-write-wins by `updatedAt` per record. **Deps: none** (new `backend/` tree; requires `.env` with DB URL + a device token — first phase to need secrets).
  - `slice-b` (frontend — outbox) — an IndexedDB outbox that records local mutations, a sync engine that flushes on `online`, and pull-merge into Dexie. **Deps: slice-a** (endpoint contract).
  - `slice-c` (frontend — status/restore) — sync-status indicator, manual "Sync now", and first-run "Restore from cloud" on a fresh device. **Deps: slice-b**.
- **Key surfaces / files:** slice-a: `backend/` (`src/main.py`, models, `alembic/`), `.env.example`; slice-b: `frontend/src/lib/sync/outbox.ts`, `frontend/src/lib/sync/engine.ts`; slice-c: `frontend/src/components/SyncStatus.tsx`, restore flow in `frontend/src/app/settings/page.tsx`.
- **Gate command:** backend from `backend/`: `uv run pytest` (against a real Postgres via `.env`, not SQLite); frontend from `frontend/`: `pnpm test && pnpm build && pnpm test:e2e` (E2E: create offline → go online → assert push; wipe local → restore → assert bills reappear, against the real running FastAPI service).
- **How the user tests it (handoff seed):** Create a bill offline; reconnect; see the sync indicator flush to "synced". On a second device, enter the same device token and **Restore** — all bills/farmers/payments reappear. This is the first phase needing `.env` (backend DB URL + device token); a `backend/.env.example` documents the keys.
