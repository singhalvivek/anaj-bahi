...
**The Firebase multi-tenant redesign (Phases 6–9) does NOT change this.** Firebase Auth (Google sign-in) and Cloud Firestore replace the retired FastAPI sync backend, but they are ordinary client SDKs — **still no AI, no LLM, no agent, no prompt, no model, no graph**. This file stays N/A.
