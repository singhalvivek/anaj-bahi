...
- **Roles:** `employee` may create bills (sack + quick), record payments, edit/delete bills (respecting the edit-lock), and manage farmers & custom grain types. `owner` additionally edits the business profile, invites employees (by mobile via a one-time code; the employee's name comes from their own Google login at claim) and removes them, and reads the activity log. Multiple owners per business are allowed.
