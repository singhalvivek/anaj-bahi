...
3. **Real-key check** (Phase 2+) — the gate runs against the REAL LLM/API using keys from `.env`, and against the **production database engine** (not a lighter substitute, e.g. SQLite if prod is PostgreSQL). A required key missing from `.env` → BLOCKED with the exact key name. Never substitute a lighter engine for the production database.
4. **Golden-path + live-server + UI smoke** (phase-level; run once at aggregation — see Scope). Three required sub-steps:

   **4a. Boot gate (REQUIRED, runs before any curl) — the test path MUST equal the run path.** Start the app via the **EXACT "Dev run command"** from `spec/architecture.md` → `## Commands` (mirrored in the README/roadmap), from the **project root**, and confirm it **boots with no import/module/startup error** before doing anything else. A green test run does NOT satisfy this: test runners commonly put `.` on the path, so run-path import bugs pass the test suite and only fail on the documented command. A server that does not boot on its own documented command is a **BLOCKER**, even with green tests. *(Worked example — Python: `uv run python -m src` / `uv run uvicorn ...`; a `src.`-prefixed import that crashes on the real boot still passes pytest because pytest puts `.` on `sys.path`.)*

   **4b. Styled-render + E2E check (REQUIRED for any project with a frontend) — a 200 + HTML is NOT a pass; it does not detect missing CSS/JS or broken interactions.** Two sub-checks, both mandatory; use the project's real build + E2E commands from `## Commands`:

   *CSS check:* Run the frontend build, then **grep the built asset bundle** for real style output AND assert **no unprocessed style directive survives** in it. Gate the EXACT deployed path the user runs, not a dev-only convenience server. **An unstyled page that returns 200 is a BLOCKER, not a pass.** *(Worked example — Next.js + Tailwind: `cd frontend && pnpm build`; `grep -rE '\.(flex|grid|bg-|rounded-|text-)' frontend/out/_next/static/css/*.css` and `! grep -r '@tailwind' frontend/out`; gate `http://localhost:8001/app/`, not the `pnpm dev :3000` flow.)*

   *E2E smoke:* Run the project's E2E suite (from `## Commands` — Playwright by default) against the live app at its deployed URL. The smoke must assert the primary user journey **renders correctly, is interactive, and produces real output** — not just that the page loads. **The E2E suite failing is a BLOCKER** — it catches interaction bugs, missing JS bundles, and broken API wiring that CSS-grep and curl cannot. A CSS-grep pass without an E2E pass is not a gate pass. *(Worked example — `npx playwright test` from the project root against `http://localhost:8001/app/`.)*

   **4c. Capability smoke (Phase 2+)** — run the primary user journey against the real LLM/API via an in-process client (e.g. `TestClient`) or the live server, asserting **response content** not just status; exercise edge cases and at least one full end-to-end path; for any UI surface assert rendered content and empty/loading/error states. **Derive the smoke from the phase's CAPABILITIES, not just its endpoints — test the capability, not one call to it.** If any capability the phase claims is **stateful** (persistent sessions, history, memory, conversation, accumulation, "across restart"), a single happy-path call does NOT gate it: you MUST exercise a **multi-interaction sequence** (≥2 operations in the SAME session — e.g. ask, then ask again; upload, then query, then query again) AND a **state-survival check** (reload the page and/or restart the process, then confirm prior state is still there). State-dependent bugs (detached ORM rows, stale caches, history-load crashes, session-scoping errors) are invisible to the first call and only surface on the second — a one-shot smoke that passes is not evidence the capability works.
...
**Bootstrap branch (brownfield with no spec yet).** If the spec files are still empty/placeholder (`<!-- FILL IN -->`) but real application code exists, there is no spec to audit *against* — every capability is simply undocumented. Do **not** emit a wall of DIVERGENCES against a non-existent spec. Instead report **Status: NO SPEC — BOOTSTRAP NEEDED**, note the detected stack/toolchain (via `harness/patterns/tech-stack.md` → Detection signals) and a one-line inventory of the major capabilities/surfaces you can see in the code, and route to **spec-writer in Bootstrap mode** to reverse-engineer the descriptive spec first. Normal drift auditing resumes once a spec exists. (This is the path `/zero-shot-sync` and `/zero-shot-build` use when dropping into an existing repo.)

For a normal drift audit (spec exists), read every spec file, search the codebase, compare claims to reality:
...
- **Passing a UI on a 200 + HTML check alone — never confirming it actually renders STYLED and interactive** on the deployed path the user runs. A clean build and a 200 do not prove CSS/JS loaded or interactions work. CSS-grep + E2E suite are both required.
- Handing the user (or gating) a dev-only convenience server on a different origin instead of the deployed path the app actually serves. *(Worked example — Next.js: the two-server `pnpm dev :3000` flow instead of the single-origin `pnpm build` → `:8001/app/`.)*
- Skipping the E2E suite because the CSS-grep passed — both checks are mandatory and independent.
