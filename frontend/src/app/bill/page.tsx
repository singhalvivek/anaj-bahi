...
/**
 * Share-as-image panel. A real Share button opens the reused ReceiptShareSheet —
 * a receipt-preview bottom sheet (positioned BELOW the sticky top bar so the
 * language toggle stays reachable — language of the rasterised image follows the
 * live toggle) that renders <Receipt> and shares it as a PNG via shareReceiptImage:
 *   - phone with native file share  → native share sheet ('shared')
 *   - otherwise (desktop/headless)  → PNG download + a clear fallback note
 * Never a dead button. The preview/share body lives in ReceiptShareSheet (shared
 * with the Phase-11 post-save prompt); the trigger button + DOM here are unchanged.
 */
function SharePanel({
  bill,
  farmerPhone,
